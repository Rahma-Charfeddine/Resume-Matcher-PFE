...
import os #used for interacting with the operating system (e.g., checking file existance)
...
def get_pdf_files(file_path: str) -> list:
    ...
    try:
        ...
        print(f"Error getting PDF files from '{file_path}': {str(e)}") # returns the list of file paths found in the directory
    return pdf_files

"""
    File Operations: utilizes 'os.path' and 'glob.glob' for robust file handling across different operating systems
    and file systems. 
    The 'os.path' module provides functions for manipulating file paths, while 
...
# the first function 'get_pdf_files' checks if path exists ( os.path.exists())
# no exception handling
# direclty returns file list
# no type hints



# the second function 'get_pdf_files' no explicit checks 
# uses try-except 
# stores in a variable before returning
# uses type hints for better clearity 
